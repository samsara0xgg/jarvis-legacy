...
## 2026-04-17 批次（desktop 上线后观察）

### ❌ 天气回复 "天气" 重复字
- **复现**：桌面端（Electron + ui/web/）输入「今天天气怎么样」
- **实际输出**：`今天天气天气：阴天，温度12°C（体感12°C），湿度77%，风速4公里/时。`
- **期望输出**：`今天天气：阴天，温度12°C（体感12°C），湿度77%，风速4公里/时。`
- **截图**：2026-04-17 Electron 启动验收时（小月 hiyori 模型已正常显示）
- **猜测**：
  - `skills/weather.py` 的模板字符串拼接重复了「天气」前缀（最可能）
  - 或 `core/llm.py` 的 info_query 模板化路径中有字段级重复拼接
  - 查找点：grep `"今天天气"` 或 `"天气天气"` 在 skills/ 和 core/ 下
- **影响**：P2，体验扣分但不影响功能。天气场景高频，最好修。
- **同时观察**：Log 面板有 `requests.exceptions.HTTPError: 400` ——可能是天气 API 侧面限频或 key 问题，和"天气"重复可能无关，需单独排查
