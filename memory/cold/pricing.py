...
pricing table. The table is normally loaded from ``data/pricing.json``
(generated by ``scripts/refresh_pricing.py`` from LiteLLM's community
pricing snapshot); the legacy ``llm_pricing`` block in ``config.yaml``
