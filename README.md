...
不让你打断的语音助手感觉是对抗性的。最直觉的方案——攒 500ms 麦克风音频、跑完整 ASR、查 stop 关键词——会撞上 600-700ms 的延迟下限，用户在这段时间里已经能后悔自己刚说的那句"停"。Yue 的打断路径放弃"瞬时检测"幻想，转而对齐人真正的打断方式：在小句边界、用单字标记、期待系统淡出而不是一刀切。

**架构选型。** 在五个架构层级（参数调优 → 组件替换 → 管线重设计 → UX 范式转移 → 硬件重构）下评估了 7 种方案。KWS 单独检测被否决，原因有二：中文单字同音词（`灯` vs `等`、`挺` vs `停`）造成无法消除的误触发，固定关键词词表对没见过的说法（`别说了`、`够了够了`）很脆弱。Speculative pause 和 continuous probabilistic ducking 推迟。最终上线的设计是双阶段软停 / 硬停 + 共享 SenseVoice ASR 路径，长期路线图加上 proactive yielding（句间插入静默）和 XVF3800 DOA 过滤。

**迁移故事。** 第一版用了独立的 streaming Zipformer 转录器——教科书式的低延迟 KWS 思路。4 月 17 日的基准测试讲了另一个故事：单字"停"的 speech-to-detect 中位数是 **1760ms**，因为流式 decoder 需要 500ms 缓冲音频才能 commit 一个中文单字 token，而它的训练语料对孤立音节也很稀疏。迁移把打断路径统一到主 ASR 用的 SenseVoice INT8 栈上，配合 VAD 门控的音频段。新中位数 **1179ms（-33%）**，可靠性和主 ASR loop 一致——因为它字面上就是同一个模型同一个 normalizer（构造时注入，不重新初始化）。

**管线流程。** TTS 播放期间，独立的麦克风线程把帧喂给 Silero VAD，按模式切 dBFS 阈值（`headphones` = -40 dBFS、`speakers` = -22 dBFS 抗 TTS bleed）。VAD IDLE→ACTIVE 时 `on_soft_pause` 触发，播放音量在 30ms 内 ramp 到 30%。激活段配 **500ms 预滚**（pre-roll，避免 VAD 的 required-hits 闸门吞掉"停"的初始辅音 t，否则会被识成"嗯"）和 **200ms 后滚**（post-roll，捕住尾部清擦音）。短于 150ms 的段当噪声丢弃。关闭的段异步派发到 worker 线程：SenseVoice 转录，三层 `ASRNormalizer`（手工修正 → 结构化别名 → 可选 fuzzy 匹配）清洗文本，子串匹配 `INTERRUPT_KEYWORDS = {"停", "等一下", "打住", "暂停", "等等", "你听我说", ...}` 决定是否 fire `on_interrupt`。命中后 ring buffer flush，LLM 取消生成。

**自研播放器。** Gain-ducking 软停促成了 `core/audio_stream_player.py`（485 行）——`afplay` 和 `ffplay` 在 SIGSTOP 暂停时都会出现约 300ms 的 loop-tail artifact，是 macOS CoreAudio HAL 层行为，不是播放器实现 bug。替代方案是一个长寿命 `sd.OutputStream` + 无锁 SPSC ring buffer，sample-accurate 的增益 ramp 在 PortAudio callback 内执行（无分配、无锁、GIL 保护的原子索引）。`GainRamp` 里有个微妙 bug 必须修：分母得是 `step - 1`，否则 ramp 最后一帧不会精确等于目标值，48 kHz 下 0.0015 的跳跃就是一个可听到的 -56 dBFS 咔嗒声。这个自研播放器顺带消除了老路径里每句 spawn subprocess 的 30-150ms 开销，做到句间零 gap。

**当前状态与指标。** 生产基准（WP6/WP7 之后）：单字"停" speech-to-detect **中位数 1179ms**、多字"等一下" **911ms**、p95 < 1850ms、XVF3800 输入下 30 秒受控静默的 **误触发率 0.0/s**。10818 次 callback 内软停 + unduck 音频确认干净（无 loop-tail、无咔嗒），零 underflow。54 个单元测试分布在三个文件里：ring buffer 环绕、gain ramp 边界（咔嗒声回归测试在 `test_audio_stream_player.py`）、VAD 段派发、fired-flag 生命周期、IDLE↔ACTIVE 状态机加 3 秒无关键词自动恢复 timeout。下一步：proactive yielding——在小句边界插入静默（数据上 ~80% 的自然打断本来就发生在那里）——之后等 XVF3800 硬件落地接 spatial gating。
